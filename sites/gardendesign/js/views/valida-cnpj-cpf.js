...
function icnpj(c) {
	...
	if (14 != c.length || /0{14}/.test(c)) return !1;
	for (var a = 0, b = 0; 12 > a; b += c[a] * d[++a]);
	if (c[12] != (2 > (b %= 11) ? 0 : 11 - b)) return !1;
	for (b = a = 0; 12 >= a; b += c[a] * d[a++]);
	return c[13] != (2 > (b %= 11) ? 0 : 11 - b) ? !1 : !0
};
...
function icpf(c) {
	...
	for (i = 1; i <= 9; i++) {
		s = s + parseInt(c.substring(i - 1, i)) * (11 - i)
	}
	r = s % 11;
	if (r == 10 || r == 11 || r < 2) {
		r = 0
	...
	if (r != parseInt(c.substring(9, 10))) {
		return false
	}
	s = 0;
	for (i = 1; i <= 10; i++) {
		s = s + parseInt(c.substring(i - 1, i)) * (12 - i)
	}
	r = s % 11;
	if (r == 10 || r == 11 || r < 2) {
		r = 0
	...
	if (r != parseInt(c.substring(10, 11))) {
		return false
	}
	return true
}
...
$(document).ready(function () {
	var options = {
		...
		onChange: function (telefone) {
			console.log('telefone changed! ', telefone.length);
		}
	};
	$('.celular').mask('(00) 0000-0000', options, {
		'translation': {
			0: {
				pattern: /[0-9*]/
			}
